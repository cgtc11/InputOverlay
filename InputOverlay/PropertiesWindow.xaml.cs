...
namespace InputOverlay
{
    public partial class PropertiesWindow : Window
    {
        ...
        private void Apply_Changed(object sender, EventArgs e)
        {
            ...
            bgColor.A = (byte)(Math.Max(0, Math.Min(1, alpha)) * 255);
            main.OverlayBorder.Background = new SolidColorBrush(bgColor);

            // 影設定
            if (FindName("ChkShadowEnabled") is CheckBox chk)
                main.ShadowEnabled = chk.IsChecked == true;
            if (FindName("ShadowOffsetSlider") is Slider off)
                main.ShadowOffset = Math.Max(0, off.Value);
            ...
            if (ColorPicker.SelectedItem is ComboBoxItem item)
            {
                try
                {
                    baseTextColor = (Color)ColorConverter.ConvertFromString(item.Tag.ToString());
                }
                catch { baseTextColor = Colors.White; }
            }
            double txtAlpha = main.TextOpacity;
            if (FindName("TextOpacitySlider") is Slider txt)
                txtAlpha = txt.Value;

            main.ApplyTextBrush(baseTextColor, txtAlpha); // 内部で影も更新
